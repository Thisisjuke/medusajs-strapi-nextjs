...
import { Dialog, DialogBody, DialogFooter, Box, Loader, Layout, HeaderLayout, Button, ActionLayout, Typography, TextInput, ContentLayout, EmptyStateLayout, GridLayout, FieldInput } from '@strapi/design-system';
  ...
  return (
    <div>
      ...
      >
              ...
              <>
                ...
                <ContentLayout>
                  <Box paddingTop={0} paddingBottom={4}>
                    <Typography>{(Array.isArray(selectedProducts) && selectedProducts.length) || 0} product(s)
                      selected.</Typography>
                  </Box>
                </ContentLayout>
                <ContentLayout>
                    ...
                    <GridLayout>
                      {productsData?.products.map(product => {
                        return (product.title.includes(searchValue) || product.description.includes(searchValue) ?
                            (<Box style={{gridTemplateColumns: 'repeat(3, minmax(0, 1fr)'}} padding={4} hasRadius
                                  background="neutral0" key={product.id} shadow="tableShadow">
                              <ProductCard
                                title={product.title}
        ...
        <DialogFooter startAction={<Button onClick={() => setIsDetailsVisible(false)} variant="tertiary">
          Cancel
        </Button>} endAction={<Button variant="danger-light">
          Confirm
        </Button>} />
      </Dialog>
    </div>
